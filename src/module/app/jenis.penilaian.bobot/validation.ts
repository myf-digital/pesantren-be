...
const bobotSchema = z.object({
    ...
    lembaga_type: z.enum(['FORMAL', 'PESANTREN'], 'lembaga_type wajib FORMAL/PESANTREN'),
    ...
    bobot: z.number().min(0, 'bobot harus ≥ 0').max(100, 'bobot tidak boleh > 100'),
    status: z.enum(['Aktif', 'Nonaktif']).optional()
});

export const bulkBobotSchema = z.array(bobotSchema).nonempty('Input minimal 1 data');
...
export const updateExistingBobot = async (
     { lembaga_type, id_lembaga, id_tingkat, id_tahunajaran, bobot, id_penilaian, status = "Aktif" }: any
): Promise<boolean> => {
...
export const getTotalBobot = async (
    { lembaga_type, id_lembaga, id_tingkat, id_tahunajaran, bobot, id_penilaian, status = "Aktif" }: TotalBobotParams
): Promise<number> => {
    const whereCondition: any = { lembaga_type, id_lembaga, id_tahunajaran, status };
    if (id_tingkat) whereCondition.id_tingkat = id_tingkat;
    else whereCondition.id_tingkat = { [Op.is]: null };

    const existing = await JenisPenilaianBobot.findOne({ where: {...whereCondition, id_penilaian} });
    ...
    const result = await JenisPenilaianBobot.findOne({
        attributes: [
            [fn('COALESCE', fn('SUM', col('bobot')), 0), 'total_bobot']
        ],
...
export const validateBobot = async (payload: any[], onlySchema: boolean = false) => {
    // Validasi schema
    let parsed;
