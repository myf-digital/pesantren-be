'use strict';

export default class DataMenu {
  public menu() {
    return [
      {
        id: 1,
        menu_name: 'Dashboard',
        menu_icon: 'tabler-smart-home',
        module_name: '/dashboards/crm',
        seq_number: 1,
        parent_id: '00000000-0000-0000-0000-000000000000',
        status: 1,
      },
      {
        id: 2,
        menu_name: 'Master Pendidikan',
        menu_icon: 'tabler-school',
        module_name: '#',
        seq_number: 2,
        parent_id: '00000000-0000-0000-0000-000000000000',
        status: 1,
      },
      {
        id: 3,
        menu_name: 'Master Kewaliasuhan',
        menu_icon: 'tabler-book',
        module_name: '#',
        seq_number: 3,
        parent_id: '00000000-0000-0000-0000-000000000000',
        status: 1,
      },
      {
        id: 4,
        menu_name: 'Kebersihan',
        menu_icon: 'tabler-vacuum-cleaner',
        module_name: '#',
        seq_number: 4,
        parent_id: '00000000-0000-0000-0000-000000000000',
        status: 1,
      },
      {
        id: 5,
        menu_name: 'Absensi',
        menu_icon: 'tabler-clipboard-check',
        module_name: '#',
        seq_number: 5,
        parent_id: '00000000-0000-0000-0000-000000000000',
        status: 1,
      },
      {
        id: 6,
        menu_name: 'Perizinan',
        menu_icon: 'tabler-door-exit',
        module_name: '#',
        seq_number: 6,
        parent_id: '00000000-0000-0000-0000-000000000000',
        status: 1,
      },
      {
        id: 7,
        menu_name: 'Kesehatan',
        menu_icon: 'tabler-stethoscope',
        module_name: '#',
        seq_number: 6,
        parent_id: '00000000-0000-0000-0000-000000000000',
        status: 1,
      },
      {
        id: 88,
        menu_name: 'Laporan',
        menu_icon: 'tabler-report',
        module_name: '#',
        seq_number: 6,
        parent_id: '00000000-0000-0000-0000-000000000000',
        status: 1,
      },
      {
        id: 99,
        menu_name: 'Settings',
        menu_icon: 'tabler-settings',
        module_name: '#',
        seq_number: 99,
        parent_id: '00000000-0000-0000-0000-000000000000',
        status: 1,
      },
    ];
  }

  public childmenu() {
    return [
      {
        parent_id: 2,
        menu_name: 'Lembaga Formal',
        menu_icon: 'tabler-circle',
        module_name: '/app/lembaga-formal/list',
        seq_number: 1,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Lembaga Kepesantrenan',
        menu_icon: 'tabler-circle',
        module_name: '/app/lembaga-kepesantrenan/list',
        seq_number: 2,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Tingkat',
        menu_icon: 'tabler-circle',
        module_name: '/app/tingkat/list',
        seq_number: 3,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Tahun Ajaran',
        menu_icon: 'tabler-circle',
        module_name: '/app/tahun-ajaran/list',
        seq_number: 4,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Semester',
        menu_icon: 'tabler-circle',
        module_name: '/app/semester/list',
        seq_number: 5,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Status Awal Santri',
        menu_icon: 'tabler-circle',
        module_name: '/app/status-awal-santri/list',
        seq_number: 6,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Jenis Beasiswa',
        menu_icon: 'tabler-circle',
        module_name: '/app/jenis-beasiswa/list',
        seq_number: 7,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Kelompok Mata Pelajaran',
        menu_icon: 'tabler-circle',
        module_name: '/app/kelompok-pelajaran/list',
        seq_number: 8,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Mata Pelajaran',
        menu_icon: 'tabler-circle',
        module_name: '/app/mata-pelajaran/list',
        seq_number: 9,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Jenis Jam Pelajaran',
        menu_icon: 'tabler-circle',
        module_name: '/app/jenis-jam-pelajaran/list',
        seq_number: 10,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Jam Pelajaran',
        menu_icon: 'tabler-circle',
        module_name: '/app/jam-pelajaran/list',
        seq_number: 11,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Guru Mata Pelajaran',
        menu_icon: 'tabler-circle',
        module_name: '/app/guru-matapelajaran/list',
        seq_number: 12,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Kelas MDA',
        menu_icon: 'tabler-circle',
        module_name: '/app/kelas-mda/list',
        seq_number: 13,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Kelas Formal',
        menu_icon: 'tabler-circle',
        module_name: '/app/kelas-formal/list',
        seq_number: 14,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Jadwal Pelajaran',
        menu_icon: 'tabler-circle',
        module_name: '/app/jadwal-pelajaran/list',
        seq_number: 15,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Jenis Penilaian',
        menu_icon: 'tabler-circle',
        module_name: '/app/jenis-penilaian/list',
        seq_number: 16,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Jenis Bobot Penilaian',
        menu_icon: 'tabler-circle',
        module_name: '/app/jenis-penilaian-bobot/list',
        seq_number: 17,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Rapot Santri',
        menu_icon: 'tabler-circle',
        module_name: '/app/rapot-santri/list',
        seq_number: 18,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Jam Kerja Pegawai',
        menu_icon: 'tabler-circle',
        module_name: '/app/pegawai-jam-kerja/list',
        seq_number: 19,
        status: 1,
      },
      {
        parent_id: 2,
        menu_name: 'Guru Pengganti',
        menu_icon: 'tabler-circle',
        module_name: '/app/guru-pengganti/list',
        seq_number: 20,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Cabang',
        menu_icon: 'tabler-circle',
        module_name: '/app/cabang/list',
        seq_number: 1,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Organisasi',
        menu_icon: 'tabler-circle',
        module_name: '/app/organisasi/list',
        seq_number: 2,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Lokasi',
        menu_icon: 'tabler-circle',
        module_name: '/app/location/list',
        seq_number: 3,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Jabatan',
        menu_icon: 'tabler-circle',
        module_name: '/app/jabatan/list',
        seq_number: 4,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Santri',
        menu_icon: 'tabler-circle',
        module_name: '/app/santri/list',
        seq_number: 5,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Penempatan Kamar Santri',
        menu_icon: 'tabler-circle',
        module_name: '/app/penempatan-kamar-santri/list',
        seq_number: 6,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Penempatan Kelas Santri',
        menu_icon: 'tabler-circle',
        module_name: '/app/penempatan-kelas-santri/list',
        seq_number: 7,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Orang Tua / Wali',
        menu_icon: 'tabler-circle',
        module_name: '/app/orang-tua-wali/list',
        seq_number: 8,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Pegawai',
        menu_icon: 'tabler-circle',
        module_name: '/app/pegawai/list',
        seq_number: 9,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Shift Presensi',
        menu_icon: 'tabler-circle',
        module_name: '/app/shift-presensi/list',
        seq_number: 10,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Master Slot Waktu',
        menu_icon: 'tabler-circle',
        module_name: '/app/master-slot-waktu/list',
        seq_number: 11,
        status: 1,
      },
      {
        parent_id: 3,
        menu_name: 'Jadwal Inspeksi Kebersihan',
        menu_icon: 'tabler-circle',
        module_name: '/app/jadwal-inspeksi-kebersihan/list',
        seq_number: 12,
        status: 1,
      },
      {
        parent_id: 4,
        menu_name: 'Inspeksi',
        menu_icon: 'tabler-alert-circle',
        module_name: '/app/kebersihan-inspeksi/list',
        seq_number: 1,
        status: 1,
      },
      {
        parent_id: 4,
        menu_name: 'Temuan',
        menu_icon: 'tabler-list-search',
        module_name: '/app/kebersihan-temuan/list',
        seq_number: 2,
        status: 1,
      },
      {
        parent_id: 4,
        menu_name: 'Log',
        menu_icon: 'tabler-logs',
        module_name: '/app/kebersihan-scan-log/list',
        seq_number: 3,
        status: 1,
      },
      {
        parent_id: 5,
        menu_name: 'Absen Harian',
        menu_icon: 'tabler-circle',
        module_name: '/app/absen-harian-santri/list',
        seq_number: 1,
        status: 1,
      },
      {
        parent_id: 5,
        menu_name: 'Absen Kelas',
        menu_icon: 'tabler-circle',
        module_name: '/app/absen-kelas-santri/list',
        seq_number: 2,
        status: 1,
      },
      {
        parent_id: 5,
        menu_name: 'Absen Pegawai',
        menu_icon: 'tabler-circle',
        module_name: '/app/pegawai-absen-harian/list',
        seq_number: 3,
        status: 1,
      },
      {
        parent_id: 6,
        menu_name: 'Perizinan Santri',
        menu_icon: 'tabler-circle',
        module_name: '/app/perizinan-santri/kewaliasuhan',
        seq_number: 1,
        status: 1,
      },
      {
        parent_id: 6,
        menu_name: 'Perizinan Santri Massal',
        menu_icon: 'tabler-circle',
        module_name: '/app/perizinan-santri/form-massal',
        seq_number: 2,
        status: 1,
      },
      {
        parent_id: 6,
        menu_name: 'Perizinan Pegawai',
        menu_icon: 'tabler-circle',
        module_name: '/app/perizinan-pegawai/kewaliasuhan',
        seq_number: 3,
        status: 1,
      },
      {
        parent_id: 6,
        menu_name: 'Surat Izin Santri',
        menu_icon: 'tabler-circle',
        module_name: '/app/perizinan-santri/kedisiplinan',
        seq_number: 4,
        status: 1,
      },
      {
        parent_id: 6,
        menu_name: 'Surat Izin Pegawai',
        menu_icon: 'tabler-circle',
        module_name: '/app/perizinan-pegawai/kedisiplinan',
        seq_number: 5,
        status: 1,
      },
      {
        parent_id: 6,
        menu_name: 'Scan QR Gate',
        menu_icon: 'tabler-circle',
        module_name: '/app/log-gate/list',
        seq_number: 6,
        status: 1,
      },
      {
        parent_id: 7,
        menu_name: 'Kesehatan Santri',
        menu_icon: 'tabler-circle',
        module_name: '/app/kesehatan-santri/list',
        seq_number: 1,
        status: 1,
      },
      {
        parent_id: 7,
        menu_name: 'Kesehatan Pegawai',
        menu_icon: 'tabler-circle',
        module_name: '/app/kesehatan-pegawai/list',
        seq_number: 2,
        status: 1,
      },
      {
        parent_id: 88,
        menu_name: 'Inspeksi Temuan',
        menu_icon: 'tabler-circle',
        module_name: '/app/report/kebersihan-temuan/list',
        seq_number: 1,
        status: 1,
      },
      {
        parent_id: 88,
        menu_name: 'Absensi Harian',
        menu_icon: 'tabler-circle',
        module_name: '/app/report/absen-harian-santri/list',
        seq_number: 2,
        status: 1,
      },
      {
        parent_id: 88,
        menu_name: 'Absensi Kelas',
        menu_icon: 'tabler-circle',
        module_name: '/app/report/absen-kelas-santri/list',
        seq_number: 3,
        status: 1,
      },
      {
        parent_id: 88,
        menu_name: 'Perizinan-Santri',
        menu_icon: 'tabler-circle',
        module_name: '/app/report/perizinan-santri/list',
        seq_number: 4,
        status: 1,
      },
      {
        parent_id: 88,
        menu_name: 'Perizinan-Pegawai',
        menu_icon: 'tabler-circle',
        module_name: '/app/report/perizinan-pegawai/list',
        seq_number: 5,
        status: 1,
      },
      {
        parent_id: 88,
        menu_name: 'Petugas Kebersihan',
        menu_icon: 'tabler-circle',
        module_name: '/app/report/kebersihan-petugas/list',
        seq_number: 6,
        status: 1,
      },
      {
        parent_id: 88,
        menu_name: 'Absensi Pegawai',
        menu_icon: 'tabler-circle',
        module_name: '/app/report/absen-harian-pegawai/list',
        seq_number: 7,
        status: 1,
      },
      {
        parent_id: 88,
        menu_name: 'Rekap Jadwal Guru',
        menu_icon: 'tabler-circle',
        module_name: '/app/report/rekap-jadwal-guru/list',
        seq_number: 8,
        status: 1,
      },
      {
        parent_id: 88,
        menu_name: 'Activity User',
        menu_icon: 'tabler-circle',
        module_name: '/app/report/activity-log/list',
        seq_number: 99,
        status: 1,
      },
      {
        parent_id: 99,
        menu_name: 'User',
        menu_icon: 'tabler-circle',
        module_name: '/app/user/list',
        seq_number: 1,
        status: 1,
      },
      {
        parent_id: 99,
        menu_name: 'Role',
        menu_icon: 'tabler-circle',
        module_name: '/app/role/list',
        seq_number: 2,
        status: 1,
      },
      {
        parent_id: 99,
        menu_name: 'Menu',
        menu_icon: 'tabler-circle',
        module_name: '/app/menu/list',
        seq_number: 3,
        status: 1,
      },
      {
        parent_id: 99,
        menu_name: 'Param Global',
        menu_icon: 'tabler-circle',
        module_name: '/app/param-global/list',
        seq_number: 4,
        status: 1,
      },
    ];
  }
}
export const datamenu = new DataMenu();
